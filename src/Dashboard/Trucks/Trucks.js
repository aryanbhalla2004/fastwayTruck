            ...
            <tbody>
              {!loading && (trucks && trucks.map((item, index) => (
                <tr key={index}>
                  ...
                  <td>{item.quantity}</td>
                  <td><a href="#" className="btn-danger delete-button-table" onClick={() => {setDeleteBox(true); setDeleteId({...item, type: "Trucks"})}}><i class="bi bi-trash3"></i> Delete</a><Link className="edit-button" to={`/dashboard/trucks/${item.id}`}><i class="bi bi-binoculars"></i> View</Link></td>
                </tr>
              )))}
            </tbody>
